...
// Additional images for team/profile
const profileImages = [
  { src: 'BKunkeExp.jpg', maxWidth: 300 },
  { src: 'LinkedIn_Aug072025.jpeg', maxWidth: 300 },
  { src: 'Omnie.jpg', maxWidth: 600 }
];
...
async function optimizeImages() {
  ...
  for (const img of profileImages) {
    ...
    const outputPath = path.join(imagesDir, img.src.replace(/\.(jpg|jpeg)$/i, '.webp'));
    ...
    await sharp(inputPath)
      .resize(img.maxWidth, img.maxWidth, { fit: 'inside', withoutEnlargement: true })
      .webp({ quality: 85 })
      .toFile(outputPath);
    ...
    const saved = originalSize - newSize;
    totalSaved += saved;
    console.log(`${img.src} -> WebP: ${(originalSize/1024).toFixed(0)}KB -> ${(newSize/1024).toFixed(0)}KB (saved ${(saved/1024).toFixed(0)}KB)`);
  }
